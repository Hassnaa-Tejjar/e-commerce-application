 ...
 const DescProduct=({prod})=>{
    return (
          ...
          <View
           ...
           <Text style={{fontSize:15,color:'rgba(0,0,0,0.5)'}}>{prod.quantite}</Text>
           </View>
           <View
              style={{
                marginLeft: 20,
   ...
   return(
     ...
     <FlatList
        ...
        keyExtractor={item=>item.id}
      /></View>
      <View style={{backgroundColor:'#FFFFFF',flexDirection:'row',alignContent:'space-between'}}>
      <View style={style.addcommentBorder}>
      <TextInput  onChangeText={(text)=>{setText(text)}} placeholder="        ajouter un commentaire"/>
      <Text>{text}</Text>
      </View>
      <View style={[style.addcommBtn]}>
        <Text > Add</Text>
      </View>
      </View>
        <View
